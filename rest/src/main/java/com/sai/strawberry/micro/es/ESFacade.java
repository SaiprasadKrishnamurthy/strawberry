...
public class ESFacade {
    ...
    public Void init(final boolean forceRecreateEsIndex, final EventStreamConfig config) throws Exception {
        ...
        try {
            if (forceRecreateEsIndex) {
                try {
                    restTemplate.delete(appProperties.getEsUrl() + "/" + config.getConfigId());
            ...
            if (isIndexMissing(restTemplate, config)) {
                ...
                // create index.
                restTemplate.postForObject(appProperties.getEsUrl() + "/" + config.getConfigId(), "{}", Map.class, Collections.emptyMap());
                ...
                if (config.getIndexDefinition() != null && !config.getIndexDefinition().isEmpty()) {
                    restTemplate.postForObject(appProperties.getEsUrl() + "/" + config.getConfigId() + "/_mapping/" + config.getConfigId(), JSONSERIALIZER.writeValueAsString(config.getIndexDefinition()), Map.class, Collections.emptyMap());
                }
                LOG.info("\n\n");
    ...
    private boolean isIndexMissing(final RestTemplate restTemplate, final EventStreamConfig config) {
        try {
            restTemplate.headForHeaders(appProperties.getEsUrl() + "/" + config.getConfigId());
        } catch (Exception ex) {
